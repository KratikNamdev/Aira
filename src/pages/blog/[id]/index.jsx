...
const image =
  "https://aira.fr/wp-content/uploads/elementor/thumbs/Untitled-design-2022-06-08T163035.560-q1ma2yi36u5t2iq96fdfepmf58unjb8u6ss2xl6hls.png";
...
const Index = () => {
  ...
  const getData = async () => {
    let resp = await fetch(
      `https://aira-zij7.onrender.com/blog/getBlogs?slug=${id}`
    );
    // let resp = await fetch(`http://localhost:5000/blog/getBlogs?slug=${id}`);
        ...
        <div className="blogd">
          ...
          <div className="blogd2">
            <div className="blogd21">
              <p>{`${new Date(Number(data?.ts)).toLocaleDateString("en-US", {
                month: "long",
              })} ${new Date(Number(data?.ts)).getDate()}, ${new Date(
                Number(data?.ts)
              ).getFullYear()}`}</p>
            </div>
            <div className="blogd21">
