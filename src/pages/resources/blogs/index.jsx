...
const Index = () => {
  ...
  const getData = async () => {
    ...
    try {
      let resp = await fetch(`https://aira-zij7.onrender.com/blog/getBlogs?status=true&category=${value.category}&type=${value.type}`);
  ...
  return (
    <Box sx={{ backgroundColor: "##F8F9FB", position: "relative", pt: 9 }}>
      ...
      <Container maxWidth="lg">
      {loading && <CircularProgress sx={{ display: 'block', margin: 'auto', color: '#FFAF00' }} />} {/* Show loader if data is loading */}
        {!loading && (
